 ...
 * as the file of hex we receive it in .txt extension */
u8 AsciToHex(u8 Copy_u8Asci)
{
	u8 Result;
	if ( (Copy_u8Asci >= 48) && (Copy_u8Asci <= 57) ) 	/* from 0 : 9 */
	{
		/* ex: ascii(0) - 48 = 48 - 48 = 0 */
		Result = Copy_u8Asci - 48;
	}
	else												/* from A : F (see the ascii code table) */
	{
		/* ex: ascii(A) - 55 = 65 - 55 = 10 */
...
void ParseData(u8* Copy_u8BufData)
{
	...
	/* Copy_u8BufData[7] & Copy_u8BufData[8] ---> two digits for RT(recored type) */
	/* CC = no of Recored bytes
	 * every byte = 2 digit  so i<CC/2 */
	for (i=0;i<CC/2; i++)
	{
